    ...
    try {
        ...
        form.addEventListener("submit", async (event) => {
            ...
            try {
                ...
                // Successful login: server issues a redirect — go there immediately (no success modal)
                if (response.redirected) {
                    window.location.href = response.url;
                    return;
                }

                // All auth failures use the same generic message (server also returns this)
