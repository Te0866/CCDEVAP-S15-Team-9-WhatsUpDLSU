<?php
    ...
    $sql = "SELECT EVENT_ID, TITLE, DATE, LOCATION, VENUE, CATEGORY, APPROVAL_STATUS FROM event WHERE ORG_ID = ?";
    $params = array($orgId);
    $paramTypes = "i";
    ...
    $stmt2 = mysqli_prepare($conn, "SELECT APPROVAL_STATUS, COUNT(*) AS total FROM event WHERE ORG_ID = ? GROUP BY APPROVAL_STATUS");
    mysqli_stmt_bind_param($stmt2, "i", $orgId);
