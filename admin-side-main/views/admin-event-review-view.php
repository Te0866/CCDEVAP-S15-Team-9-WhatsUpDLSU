...
$approvalStatus = strtolower($event['APPROVAL_STATUS']);
...
if (!empty($event['BANNER_IMAGE'])) {
    foreach (explode(',', $event['BANNER_IMAGE']) as $img) {
        $img = trim($img);
        if ($img !== '') {
            $bannerImages[] = '../org-side-main/uploads/' . str_replace('%2F', '/', rawurlencode($img));
        }
    }
}
?>
...
<link rel="stylesheet" href="../assets/styles/admin/admin-event-review.css">
    </head>

    <body class="s-admin-admin-event-review s-admin-darkmode">
        <nav class="navbar">
            <div class="nav-left">
        ...
        <main class="review-page">
            <form class="action-panel" method="POST" action="update-event-status.php">
                ...
                <button class="approve-btn" id="approveBtn" type="submit" name="action" value="approve" <?php echo $approvalStatus === 'approved' ? 'disabled' : ''; ?>> Approve </button>
                <button class="reject-btn" id="rejectBtn" type="submit" name="action" value="reject" <?php echo $approvalStatus === 'rejected' ? 'disabled' : ''; ?>> Reject </button>
            </form>
        ...
        <script>
            const eventImages = <?php echo json_encode($bannerImages); ?>;
        </script>
<script src="../assets/scripts/admin/darkmode.js"></script>
<script src="../assets/scripts/admin/admin-event-review.js"></script>
